import { formatTime, formatUptime, STATUS_HEX, STATUS_LABEL, type Status } from "@/lib/system-health/telemetry";
...
export function TopBar({ now, bootedAt, lastSync, global, buildHash = "a1f3c2e", region = "br-sa1", env = "prod" }: Props) {
    ...
    <header className="flex items-center justify-between border-b border-[#1F2330] bg-[#0A0B0F]/80 px-5 py-3">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-baseline gap-2">
          ...
          <span className="text-[11px] uppercase tracking-[0.14em] text-[#7A8194]">System Health</span>
        </div>
      </div>
      <div className="flex items-center gap-5 font-mono text-[11px] tabular-nums text-[#7A8194]">
        <Pill label="env" value={env} />
        <Pill label="region" value={region} />
        <Pill label="build" value={buildHash} />
