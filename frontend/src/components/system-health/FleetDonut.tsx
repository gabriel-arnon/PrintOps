      ...
      <div className="flex items-center gap-5">
        <div className="relative h-[140px] w-[140px] shrink-0">
          ...
          <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
            <span className="font-mono text-[10px] uppercase tracking-wider text-[#525a6e]">online</span>
            <span className="font-mono text-xl tabular-nums text-[#E6E8EE]">
              {((fleet.online / total) * 100).toFixed(1)}%
            </span>
          </div>
        </div>
        <ul className="flex-1 space-y-2">
          {data
            .filter((d) => d.value > 0)
            .map((d) => (
              <li key={d.name} className="flex items-center justify-between border-b border-[#1F2330]/60 pb-1.5">
