import { formatCountdown, formatTime, STATUS_HEX, STATUS_LABEL, type PollingInfo } from "@/lib/system-health/telemetry";
...
export function PollingMetrics({ polling }: { polling: PollingInfo }) {
  ...
  const rows: Array<[string, React.ReactNode]> = [
    ...
    ["next run", `in ${formatCountdown(polling.nextRunInSec)}`],
  ...
  return (
    <section className="flex h-full flex-col border border-[#1F2330] bg-[#11131A]">
      <header className="flex items-center justify-between border-b border-[#1F2330] px-4 py-2.5">
        <span className="text-[11px] uppercase tracking-[0.14em] text-[#7A8194]">Polling Metrics</span>
        <span className="font-mono text-[11px] tabular-nums text-[#525a6e]">snmp-poller-01</span>
      </header>
      <dl className="grid flex-1 grid-cols-1 gap-y-1.5 p-4">
        {rows.map(([k, v]) => (
          <div key={k} className="flex items-center justify-between border-b border-[#1F2330]/60 pb-2 last:border-0 last:pb-0">
